# z_scout.md — ESP32 (S3 / C5) Zenoh 网络发现（Scout）教程
...
`main/z_scout.c` 是一个面向 **ESP32 (S3 / C5)** 的 Zenoh **网络发现（Scout）** 示例程序，运行于 **ESP-IDF v6.0** 框架之上。它通过发送 UDP 多播查询来发现局域网中的所有 Zenoh 节点（路由器、对等端、客户端），并以格式化方式打印每个节点的 `Hello` 回复。
...
| 宏 | 含义 |
|----|------|
| `ESP_WIFI_SSID` | WiFi 热点名称（2.4 GHz — ESP32-S3 不支持 5 GHz；ESP32-C5 支持 5 GHz 但本例配置为 2.4 GHz） |
...
在同一网络上运行另一块 ESP32 (S3 / C5) 的 `z_pub.c` 或 `z_sub.c`（如果它们处于 peer 模式或使用 scout 查找路由器），它们也会被 Scout 发现。
...
按 ESP32 (S3 / C5) 的复位按钮，或在监视器中按 `Ctrl+T` → `Ctrl+Y` 复位板子。
